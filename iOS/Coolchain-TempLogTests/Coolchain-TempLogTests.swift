import XCTest
@testable import Coolchain-TempLog

@MainActor
final class Coolchain-TempLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = Reading(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Reading(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(Reading(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Reading(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(Reading(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = Reading(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testUpdateChangesTitle() {
        var item = Reading(title: "Original")
        _ = store.add(item, isPro: false)
        item.title = "Updated"
        store.update(item)
    ...
    func testPersistenceRoundTrip() {
        let item = Reading(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
